...
double start_time, end_time, elapsed_time;
    ...
    int NUMBER_STEPS_X=200;
    int NUMBER_STEPS_Y=200;
    ...
    // Compute optimal omega using theoretical formula for 2D Laplace equation
    // This is independent of domain decomposition and processor count
    double omega_opt;
    if(NUMBER_STEPS_X == 200) omega_opt = 1.96907;
    else if(NUMBER_STEPS_X == 120) omega_opt = 1.94898;
    else if(NUMBER_STEPS_X == 80) omega_opt = 1.92444;
    
    // Broadcast omega to all processes
    MPI_Barrier(MPI_COMM_WORLD);

    // Data movement vectors for all processes..
    long long int NUMBER_ITERATION_MAX = 100000000; 
    ...
    double  row_error, local_error, global_error, global_error_max = 1e-6, sum;
     start_time = MPI_Wtime(); 
    // local Matrix solver (SOR with optimal omega)..
for (k = 1; k <= NUMBER_ITERATION_MAX; k++) {
    ...
    // SOR solver for each process..
    // SOR formula: x_new = (1-omega)*x_old + omega*(b - R*x_old)/a_ii
    local_error = 0.0;
    for (j = 1; j <= elements; j++) {
        sum = 0.0;
        ...
        double x_jacobi = (B[j] - sum) / A[j][j];
        X[j] = (1.0 - omega_opt) * X_old[j] + omega_opt * x_jacobi;
    ...
    if (global_error <= global_error_max){
        if(world_rank == 0) printf("Iteration #%d: \t Error: %e \n", k, global_error);
    	break;
    }
}
    end_time = MPI_Wtime();
    ...
    if (world_rank == 0){ 
        ...
        printf("Using SOR with optimal omega = %.6f\n", omega_opt);
	printf("Time elapsed for the calculation: %f seconds\n", elapsed_time);
	}
   MPI_Finalize();
}
